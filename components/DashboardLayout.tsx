...
export default function DashboardLayout({
  ...
  useEffect(() => {
    if (!loading) {
      ...
      if (requiredRole === 'admin' && !isAdmin) {
        router.push("/kits"); // Redirect non-admins to their library
      }
    }
  }, [user, loading, isAdmin, isPartner, router, requiredRole]);
  ...
  const roleTitle = isAdmin ? "Admin" : isPartner ? "Partner" : "User";
  const roleSubtitle = isAdmin ? "Panel" : isPartner ? "Portal" : "Dashboard";
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
        ...
        <header className="lg:hidden h-16 border-b border-border bg-card flex items-center justify-between px-4 sticky top-0 z-40">
          <div className="flex items-center gap-2">
             ...
             <span className="font-bold tracking-tight">{roleTitle} <span className="text-primary italic">{roleSubtitle}</span></span>
          </div>
